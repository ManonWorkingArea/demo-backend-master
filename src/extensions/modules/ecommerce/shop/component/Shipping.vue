<template>
        ...
        <template v-for="(shipping) in flattenedShippings">
          <div v-if="shipping.type === 'main'" :key="shipping._id" class="bg-white p-5 rounded-sm shadow-sm">
            ...
            <div class="flex justify-end mt-4">
              <button @click="openRenameDialog(shipping)" class="text-sm text-gray-500 hover:text-gray-700"><font-awesome-icon :icon="['fas','edit']" class="pl-1"/> Edit Data</button>
              <button @click="removeMainCategoryConfirmation(shipping)" class="text-sm text-red-500 hover:text-red-700"><font-awesome-icon :icon="['fas','trash']" class="pl-1"/> Delete</button>
            </div>
          </div>
        </template>
      </div>
    ...
    <div v-if="showRenameDialog" class="fixed inset-0 flex items-center justify-center z-10">
      ...
      <div class="bg-white w-[500px] z-10 max-w-md mx-auto">
        ...
        <div class="p-4">
          <h4 class="text-1xl font-semibold mb-4">Edit {{ selectedShipping.type === 'main' ? 'Shipping' : 'Subshipping' }} Data</h4>
          <div class="space-y-4">
            <div>
          ...
          <div class="flex justify-end mt-6 space-x-4">
            <button @click="renameShipping(selectedShipping)" class="px-6 py-2 text-sm font-medium text-white bg-blue-500 rounded-sm hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500"><font-awesome-icon :icon="['fas','save']" class="pr-2 pl-2"/> Save Data</button>
  ...
  export default {
    ...
    computed: {
        ...
        getMainShippings() {
            if (!this.shippings || !Array.isArray(this.shippings)) {
            return [];
            }
            return this.shippings.filter(shipping => shipping && shipping.type === 'main');
        },
    },
    async mounted() {
      try {
    ...
    methods: {
      ...
      async updateBanner(banner) {
        try {
          ...
          const resAPI = await fetch(`https://gateway.cloudrestfulapi.com/api/shop_shipping/${shipping._id}`, {
            method: 'PUT',
      ...
      async getShippingData() {
        const resAPI = await fetch("https://gateway.cloudrestfulapi.com/api/shop_shipping/query", {
          method: 'POST',
          headers: { 'Content-Type': 'application/json','client-token-key':this.configs.key },
          body: JSON.stringify({
            method: 'find',
      ...
      async addShipping(mode, shipping) {
        try {
          const requestData = {
            ...
            name: 'New Shipping',
            code: 'shipping-code',
            description: 'Description for New Shipping',
            type: 'main',
            ...shipping,
          };
          delete requestData._id;
  
          const resAPI = await fetch("https://gateway.cloudrestfulapi.com/api/shop_shipping", {
            method: 'POST',
            headers: { 'Content-Type': 'application/json','client-token-key':this.configs.key },
            body: JSON.stringify({
              data: requestData,
          ...
          if (resAPI.ok) {
            const newShipping = await resAPI.json();
            await this.getShippingData();
            this.shippings.push(newShipping.data);
          }
        } catch (error) {
          console.log(error);
        }
        ...
        try {
          ...
          if (resAPI.ok) {
            this.$swal({
              ...
              timer: 3000,
              icon: 'success',
              title: 'Shipping Renamed',
              text: 'Shipping renamed successfully',
            });
      ...
      async removeMainShipping() {
        try {
          if (this.categoryToRemove) {
            const resAPI = await fetch(`https://gateway.cloudrestfulapi.com/api/shop_shipping/${this.categoryToRemove._id}`, {
              method: 'DELETE',
              headers: { 'Content-Type': 'application/json','client-token-key':this.configs.key },
            });
  
            if (resAPI.ok) {
              this.shippings = this.shippings.filter(shipping => shipping._id !== this.categoryToRemove._id);
              this.$swal({
                toast: true,
                position: 'bottom-end',
                showConfirmButton: false,
                ...
                text: 'Shipping removed successfully',
              });
              this.cancelRemoveMainCategory();
            }
          }
        } catch (error) {
          console.log(error);
      ...
      removeMainCategoryConfirmation(shipping) {
        this.categoryToRemove = shipping;
      ...
      getMainShippingIndex(shipping) {
        ...
        const index = mainShippings.findIndex(item => item._id === shipping._id);
        return index !== -1 ? index + 1 : '';
      },
      flattenShippings(shipList) {
        return shipList.reduce((acc, ship) => {
          return [...acc, ship];
        }, []);
      },
    },
