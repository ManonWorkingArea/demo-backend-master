      ...
      async getChildData() {
        if(storageManager.get('session','login')) {
          try {
            ...
            let accessToken       = storageManager.get('session','token')
            let item              = this.$route.params.id;
            const resAPI          = await fetch("https://asia-southeast2-elearning-6871c.cloudfunctions.net/schools/getDataAdmin/" + item,
            {
              method: 'GET',
              headers: {'Content-Type': 'application/json','Authorization': 'Bearer ' + accessToken},
            });
...
<template>
  ...
  <div class="pt-8 mb-8 bg-white border-b" v-if="loading_sources">
        ...
        <div>
            ...
            <div class="mt-2 md:flex md:items-center md:justify-between pb-5">
              <div class="min-w-0 flex-1">
                <h2 class="text-2xl font-bold leading-7 text-gray-900 sm:truncate sm:text-3xl sm:tracking-tight"><font-awesome-icon :icon="['fas','school']" class="text-gray-500 text-[24px]"/> ข้อมูลโรงเรียน</h2>
              </div>
              <div class="mt-4 flex flex-shrink-0 md:mt-0 md:ml-4">
                <button @click="$router.push('/' + ParentPage)" type="button" class="ml-3 inline-flex items-center rounded-md border border-transparent bg-indigo-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2"><font-awesome-icon :icon="['fas','chevron-left']" class="text-gray-200 mr-2"/> ย้อนกลับ</button>
              </div>
            </div>
          </div>
          ...
          <div class="border-t border-gray-200 px-4 py-5 sm:px-6">
            <dl class="grid grid-cols-1 gap-x-4 gap-y-8 sm:grid-cols-2">
              ...
              <div class="sm:col-span-1">
                <dt class="text-sm font-medium text-gray-500">รหัสโรงเรียน</dt>
                <dd class="mt-1 text-sm text-gray-900">{{ schoolData.code }}</dd>
              </div>

              <div class="sm:col-span-1">
                <dt class="text-sm font-medium text-gray-500">Website</dt>
                <dd class="mt-1 text-sm text-gray-900">{{ schoolData.website }}</dd>
              </div>

              <div class="sm:col-span-1">
                <dt class="text-sm font-medium text-gray-500">Phone</dt>
                <dd class="mt-1 text-sm text-gray-900">{{ schoolData.phone }}</dd>
              </div>

              <div class="sm:col-span-1">
                <dt class="text-sm font-medium text-gray-500">Email</dt>
                <dd class="mt-1 text-sm text-gray-900">{{ schoolData.email }}</dd>
              </div>
              
              <div class="sm:col-span-2">
                <dt class="text-sm font-medium text-gray-500">About</dt>
                <dd class="mt-1 text-sm text-gray-900">{{ schoolData.description }}</dd>
              </div>
      ...
      <section aria-labelledby="notes-title">
        <div class="bg-white shadow sm:overflow-hidden sm:rounded-lg">
          <div class="divide-y divide-gray-200">
            <div class="px-4 py-5 sm:px-6">
              <h2 id="notes-title" class="text-lg font-medium text-gray-900">หลักสูตร</h2>
            </div>
