...
export default {
  ...
  const paidItems = this.listItems.filter(item => item.order.status === 'ชำระเงินแล้ว' || item.order.status === 'รออนุมัติ' || item.order.status === 'ลงทะเบียนคอร์ส' || item.order.status === 'สั่งซื้อ');

  const dataToExport = paidItems.map((item, index) => {
    const rowData = {
      ...
      "หมายเลขใบรับรอง": "66/03/" + item.user.citizen,
      "วันออกใบรับรอง": item.order.enroll.analytics.option.exam_round === "first" ? "29 ส.ค. พ.ศ. 2566" : "14 ก.ย. พ.ศ. 2566",
      "วันหมดอายุ": item.order.enroll.analytics.option.exam_round === "first" ? "28 ส.ค. พ.ศ. 2571" : "13 ก.ย. พ.ศ. 2571" ,
      "Certification URL": "https://doa.fti.or.th/certification-force?token="+item.user.token+"&session=224",
      ...
      "วันที่สั่งซื้อ": item.order.adddate ? item.order.adddate : "",
      "วันที่อนุมัติ": item.order.updatedAt ? item.order.updatedAt : "",

      "ใบเสร็จ": item.order.bill === "online" ? "ออนไลน์" : "ตัวจริง",
      "ประเภทผู้ซื้อ": item.order.receipt === "personal" ? "บุคคลธรรมดา" : "นิติบุคคล",
      
      "คะแนนก่อนเรียน": item.order.enroll.analytics.pre?.score ? item.order.enroll.analytics.pre?.score :'0',
      "คะแนนหลังเรียน": item.order.enroll.analytics.post?.score ? item.order.enroll.analytics.post?.score :'0',
      "คะแนนสอบซ่อม": item.order.enroll.analytics.retest?.score ? item.order.enroll.analytics.retest?.score :'0',
      "ผลการเรียน": (item.order.enroll.analytics.post?.score || item.order.enroll.analytics.retest?.score)
        ? (item.order.enroll.analytics.post?.score ?? 0) > 32 ||
          (item.order.enroll.analytics.retest?.score ?? 0) > 32
          ? 'ผ่านการประเมิน'
          : 'ไม่ผ่านการประเมิน'
        : 'ไม่ได้สอบ',
    };
  ...
  document.body.removeChild(link);
        ...
        try {
            if (this.login) {
              ...
              const pipeline = [
                ...
                // Add $match stage to filter out documents where enrollID exists
                {
                  $match: {
                    enrollID: { $exists: true, $ne: null },
                  },
                },
                {
                  $lookup: {
      ...
      async renderEnrollFirebase() {
        ...
        for (const item of this.listItems) {
          counter++;
          await this.createUserCourse(item.user.token, '224',item.order.ref_no);
      ...
      <div class="mt-4">
        ...
        <div class="mt-5 bg-white shadow ring-1 ring-black ring-opacity-5 mb-5">
          ...
          <div class="bg-white min-h-[50px] relative" :data-content="'กำลังติดต่อฐานข้อมูล กรุณารอสักครู่.....'" :class="[(activeBlock?'isblock' : 'isunblock')]">
            ...
            <template v-for="(order, index) in listItems" :key="order.user._id">
              ...
              <div class="flex border-b border-gray-200 text-sm hover:bg-gray-50">
                ...
                <div class="w-4/12 py-2 pl-4">
                  <router-link :to="'/student/detail/'+order.user._id+'?back=/lesson/detail/'+this.dataItem + '/enroll'" class="text-gray-700 hover:text-indigo-600">
                    <div class="flex items-center">
                      <!-- <div
                        v-if="!isImageValid(order.user.avatar_img)"
                        class="w-10 h-10 p-1 mr-2 rounded-sm bg-gray-200 flex justify-center items-center"
                      >
                        <span class="text-gray-400" v-html="order.user.firstname.charAt(0) + order.user.lastname.charAt(0)"></span>
                      </div> -->
                      <img
                        ...
                        alt="Profile Image"
                      >
                      <div class="ml-2"> <!-- Add margin to separate the image from the text -->
                        <span v-html="order.user.firstname + ' ' + order.user.lastname + ' (' + order.user.old_id + ')'"></span><br>
                        <span class="text-sm font-normal text-gray-500" v-html="order.user.token"></span>
                      </div>
                    </div>
                  </router-link>
                </div>
                ...
                <div class="w-2/12 py-2 pl-4 hidden md:block">
                  <font-awesome-icon :icon="['fas','id-card']" class="text-gray-500 mt-[3px] mr-1 text-sm"/> <span v-html="order.user.citizen"></span>
                </div>

                <div class="w-4/12 py-2 pl-4 pr-3 hidden md:block">
                  <font-awesome-icon :icon="['fas','phone']" class="text-gray-500 mt-[3px] mr-2 text-sm"/><span v-html="order.user.phone"></span><br>
                  <font-awesome-icon :icon="['fas','envelope']" class="text-gray-500 mt-[3px] mr-2 text-sm"/><span v-html="order.user.email"></span>
                </div>

                <div class="w-1/12 py-2 pl-4 pr-3 hidden md:block">
                  <span class="font-semibold text-blue-500">{{ order.user.enroll ? (order.user.enroll.length > 0 ? order.user.enroll.length : '0') : '0' }}</span>
                  <font-awesome-icon :icon="['fas','book']" class="text-gray-500 mt-[3px] ml-2 text-md"/>
                </div>

                <div class="w-2/12 py-2 pl-4 pr-3 hidden md:block">
                  {{ order.user.info.regdate ? order.user.info.regdate : (order.user.createAt ? order.user.createAt : "-not reg date-") }}
                </div>

                <div class="w-3/12 py-2 pl-4 pr-3 hidden md:block">
                  <button @click="$router.push('/lesson/edit/'+ order.user._id)" type="button" class="mr-2 inline-flex justify-center rounded-md border border-gray-300 bg-white px-1 py-1 text-xs font-medium text-gray-700 shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-gray-900 focus:ring-offset-2">
                    <font-awesome-icon :icon="['fas','square-pen']" class="text-black mt-[3px] mr-2 text-md"/>
                    <span>แก้ไข</span>
                  </button>
              
                  <!-- <button @click="deleteData(order.user._id)" type="button" class="mr-2 hidden md:inline-flex inline-flex justify-center rounded-md border border-gray-300 bg-white px-2 py-2 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-gray-900 focus:ring-offset-2">
                  ...
                  <button v-if="this.session.userID === '6425c40b28ebd01be519d7be' && !order.enroll?.analytics?.post?.score" @click="makePosttest(order)" type="button" class="mr-1 inline-flex justify-center rounded-md border border-gray-300 bg-white px-1 py-1 text-xs font-medium text-gray-700 shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-gray-900 focus:ring-offset-2">
                    <font-awesome-icon :icon="['fas','link']" class="text-black mt-[3px] mr-1 text-md"/>
                    <span>Test</span>
                  </button>

                  <!-- <button @click="createData(order.user.old_id)" type="button" class="mr-2 inline-flex justify-center rounded-md border border-gray-300 bg-white px-2 py-2 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-gray-900 focus:ring-offset-2">
              ...
              <div class="md:hidden p-2 border-b border-gray-300 bg-blue-50">
                <div class="w-full">
                  <button @click="$router.push('/lesson/edit/'+ order.user._id)" type="button" class="mr-2 inline-flex justify-center rounded-md border border-gray-300 bg-white px-2 py-2 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-gray-900 focus:ring-offset-2">
                    <font-awesome-icon :icon="['fas','square-pen']" class="text-black mt-[3px] mr-2 text-md"/>
                    <span>แก้ไข</span>
                  </button>
                  ...
                  <button @click="renderEnroll(order)" type="button" class="mr-2 inline-flex justify-center rounded-md border border-gray-300 bg-white px-2 py-2 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-gray-900 focus:ring-offset-2">
                      <font-awesome-icon :icon="['fas','link']" class="text-black mt-[3px] mr-2 text-md"/>
                      <span>Sync</span>
                  </button>

                  <button @click="makePosttest(order)" type="button" class="mr-2 inline-flex justify-center rounded-md border border-gray-300 bg-white px-2 py-2 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-gray-900 focus:ring-offset-2">
                    <font-awesome-icon :icon="['fas','link']" class="text-black mt-[3px] mr-2 text-md"/>
                    <span>Test</span>
                  </button>
                ...
                <div class="mb-1" v-if="layoutVisibility.order">
                  <div class="grid grid-cols-2 md:grid-cols-5 gap-1">
                    <div class="p-2 bg-gray-50">
                      <small class="block w-full text-gray-500">ใบเสร็จ</small>
                      {{ order.order.bill === 'online' ? 'ออนไลน์' : 'ตัวจริง' }} / {{ order.order.receipt === 'personal' ? 'บุคคล' : 'นิติ' }}
                    </div>
                    <div class="p-2 bg-gray-50">
                      <small class="block w-full text-gray-500">ยอดเงิน</small>
                      {{ order.order.price }}
                    </div>
                    <div class="p-2 bg-gray-50">
                      <small class="block w-full text-gray-500">การชำระเงิน</small>
                      {{ order.order.payment }}
                    </div>
                    <div class="p-2 bg-gray-50">
                      <small class="block w-full text-gray-500">วันที่สั่งซื้อ</small>
                      {{ order.order.adddate }}
                    </div>
                    <div class="p-2 bg-gray-50">
                ...
                <div class="" v-if="layoutVisibility.enroll">
                  <div class="grid grid-cols-2 md:grid-cols-5 gap-1">
                    ...
                    <div class="p-2 bg-gray-50">
                      ...
                      <template v-if="order.enroll?.analytics?.status === 'complete'">
                        {{ order.enroll?.analytics?.complete || 'N/A' }}
                      </template>
                      <template v-else>
                        {{ order.enroll?.analytics?.processing || 'N/A' }}
                        <span v-if="order.enroll?.analytics?.processing">
                          {{ "/ " + (order.enroll?.analytics?.total || '') }}
                        </span>
                      </template>
                    </div>                              
                    ...
                    <div class="p-2 bg-gray-50">
                      ...
                      {{ order.enroll?.analytics?.pre?.score || 'N/A' }}
                    </div>

                    <div :class="['p-2','bg-gray-50', order.enroll?.analytics?.post?.score > 32 ? 'text-green-700' : 'text-red-700']">
                      <small class="block w-full text-gray-500">หลังเรียน</small>
                      <span class="font-bold">{{ order.enroll?.analytics?.post?.score || 'N/A' }}</span>
                    </div>

                    <div :class="['p-2','bg-gray-50', order.enroll?.analytics?.retest?.score > 32 ? 'text-green-700' : 'text-red-700']">
                      <small class="block w-full text-gray-500">สอบซ่อม</small>
                      <span class="font-bold">{{ order.enroll?.analytics?.retest?.score || 'N/A' }}</span>
                    </div>
                    <div class="col-span-2 md:col-span-1" :class="['p-2', order.enroll?.analytics?.post?.score > 32 || order.enroll?.analytics?.retest?.score > 32 ? 'bg-green-100' : 'bg-red-100']">
                      <small class="block w-full text-gray-500">สถานะ</small>
                      {{ order.enroll?.analytics?.post?.score > 32 || order.enroll?.analytics?.retest?.score > 32 ? 'ผ่านการประเมิน' : 'ไม่ผ่านการประเมิน' }}
                    </div>
                  </div>
