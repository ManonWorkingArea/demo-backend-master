...
export default {
  ...
  methods: {
    ...
    async getData() {
        if(this.login) {
            try {
              ...
              const resAPI          = await fetch("https://gateway.cloudrestfulapi.com/api/questions/" + this.questionID, {
                method: 'GET',
              ...
              {
                this.answerList     = answerReturn
                this.answerCounter  = questionReturn.length

                this.qustionData      = questionReturn
                for (var key of Object.keys(this.answerList)) {
                  console.log(key + " -> " + this.answerList[key]._id)
                  this.values[this.answerList[key]._id] = this.answerList[key].detail
                }
        ...
        if(this.login) {
            try {
              this.answerList.forEach(element => {
                console.log("Answer",element._id);
                console.log(element._id + " -> " + this.values[element._id])
                this.updateAnswerData(element._id)
              });
            } catch (error) {
            console.log(error)
            }
    ...
    async updateAnswerData(id) {
        if(this.login) {
            try {
              this.activeBlock = true
              const callApi   = await fetch("https://gateway.cloudrestfulapi.com/api/answer/" + id, {
                method: 'PUT',
                headers: {'Content-Type': 'application/json','client-token-key':this.configs.key},
                body: JSON.stringify({
                  data: {
    ...
    async correctAnswerData(id) {
      if(this.login) {
            try {
              this.activeBlock = true
              const callApi   = await fetch("https://gateway.cloudrestfulapi.com/api/questions/" + this.questionID, {
                method: 'PUT',
                headers: {'Content-Type': 'application/json','client-token-key':this.configs.key},
                body: JSON.stringify({
                  data: {
  ...
  async mounted () { 
    try {
      ...
      this.accessSession  =  access.current._id;
    } catch (error) {
      console.log(Error);
    }
  },
            ...
            <div class="space-y-6 sm:px-6 lg:col-span-9 lg:px-0">
                ...
                <section aria-labelledby="payment-details-heading" class="relative pt-4" :data-content="'กำลังติดต่อฐานข้อมูล กรุณารอสักครู่.....'" :class="[(activeBlock?'isblock' : 'isunblock')]">
                    ...
                    <div class="divide-y divide-gray-200 overflow-hidden rounded-lg bg-white shadow mb-8">
                      ...
                      <div class="px-4 py-5 sm:p-6">
                        <div v-for="(answer) in answerList" :key="answer._id" class="col-span-4 sm:col-span-6">
                          <div class="col-span-4 sm:col-span-2 pb-4 border-dashed border-[1px] p-4 mb-3">

                              <form @submit.prevent="updateAnswerData(answer._id)">

                                <div class="mt-1 flex rounded-md shadow-sm">

                                  <span class="inline-flex items-center rounded-l-md border border-r-0 border-gray-300 py-2 px-3 shadow-sm focus:border-gray-900 focus:outline-none focus:ring-gray-900 sm:text-sm">
                                    <div class="flex items-center" v-if="answer._id===this.qustionData.correct">
                                      <input :id="answer._id" name="isTrueAnswer" type="radio" class="h-4 w-4 border-gray-300 text-indigo-600 focus:ring-indigo-500" :value="answer._id" @change="onChange($event)" checked>
                                      <label :for="answer._id" class="ml-3 block text-sm font-medium text-gray-700">คำตอบที่ถูก</label>
                                    </div>

                                    <div class="flex items-center" v-else>
                                      <input :id="answer._id" name="isTrueAnswer" type="radio" class="h-4 w-4 border-gray-300 text-indigo-600 focus:ring-indigo-500" :value="answer._id" @change="onChange($event)">
                                      <label :for="answer._id" class="ml-3 block text-sm font-medium text-gray-700">คำตอบที่ถูก</label>
                                    </div>
                                  </span>

                                  <input 
                                  v-model="values[answer._id]"
                                  type="text" :name="answer._id" :id="answer._id" :autocomplete="answer._id" class="block w-full min-w-0 flex-1 rounded-none rounded-r-md border-gray-300 py-2 px-3 shadow-sm focus:border-gray-900 focus:outline-none focus:ring-gray-900 sm:text-sm">
                                </div>
                                
                                <div class="mt-3">
                                    <div class="flex items-center">
                                      <button 
                                      @click="updateAnswerData(answer._id)"
                                      type="button" 
                                      ...
                                      <button 
                                      @click="deleteAnswer(this.examID, this.questionID, answer._id)"
                                      type="button" 
