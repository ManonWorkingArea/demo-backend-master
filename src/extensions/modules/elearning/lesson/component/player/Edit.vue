  ...
  methods: {
    ...
    async getData() {
        if(storageManager.get('session','login')) {
            ...
            const resAPI          = await fetch("https://gateway.cloudrestfulapi.com/api/player/" + this.dataItem, {
              method: 'GET',
              headers: {
            ...
            this.duration         = RestReturn.duration
            this.type             = RestReturn.type
    ...
    async updataData() {
        if(storageManager.get('session','login')) {
            try {
              const callApi       = await fetch("https://gateway.cloudrestfulapi.com/api/player/" + this.dataItem, {
                method: 'PUT',
                headers: {
                  'Content-Type': 'application/json','client-token-key':this.configs.key
                },
              ...
              if(callApi.status===200) {
                this.$swal({ 
                  ...
                  if (confirmed.isConfirmed) {
                    ...
                    this.$router.push('/lesson/player/detail/' + this.dataItem)
                  } else {
                    console.log("New");
                    //window.location.href = '/lesson/detail/' + this.parentID
                    this.$router.push('/lesson/detail/' + this.parentID)
                  }
                });
              }
            } catch (error) {
