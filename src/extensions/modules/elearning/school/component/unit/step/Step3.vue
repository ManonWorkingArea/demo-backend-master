<template>
  <div class="mt-6">
    <h2 id="payment-details-heading" class="text-lg font-bold leading-6 text-gray-900">Theme & Pages</h2>
  ...
  <div class="mt-6 grid grid-cols-4 gap-6">
    <div class="col-span-4">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          <label for="age2" class="block text-sm font-medium text-gray-700">Age2</label>
          <input
            v-model="localStepData.age2"
            type="text"
            name="age2"
            id="age2"
            autocomplete="age2"
            @input="validateStep('age2')"
            :class="{ 'border-red-500': isFieldInvalid('age2') }"
          ...
          >
          <div v-if="isFieldInvalid('age2')" class="text-red-500 text-xs mt-1">Age2 is required.</div>
        </div>

        <div>
          <label for="location2" class="block text-sm font-medium text-gray-700">Location2</label>
          <input
            v-model="localStepData.location2"
            type="text"
            name="location2"
            id="location2"
            autocomplete="location2"
            @input="validateStep('location2')"
            :class="{ 'border-red-500': isFieldInvalid('location2') }"
            class="mt-1 block w-full rounded-md border border-gray-300 py-2 px-3 shadow-sm focus:border-gray-900 focus:outline-none focus:ring-gray-900 sm:text-sm"
          >
          <div v-if="isFieldInvalid('location2')" class="text-red-500 text-xs mt-1">Location2 is required.</div>
        </div>
      </div>
    </div>
  </div>
