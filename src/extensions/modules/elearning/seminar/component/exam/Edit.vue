...
export default {
  ...
  methods: {
    ...
    async getData() {
        if(this.login) {
          try {
            ...
            this.total = RestReturn.data.total
            this.timer = RestReturn.data.timer
            this.score = RestReturn.data.score
            this.is_score = RestReturn.data.is_score
            this.description = RestReturn.data.description
            this.result = RestReturn.data.result
            this.result_duedate = RestReturn.data.result_duedate
            this.show = RestReturn.data.show
            this.show_duedate = RestReturn.data.show_duedate
            this.type = RestReturn.data.type

            this.checkedType = RestReturn.data.type
            this.checkedResult = RestReturn.data.result
            this.checkedShow = RestReturn.data.show

            this.is_answer_shuffle = RestReturn.data.is_answer_shuffle
            this.is_question_shuffle = RestReturn.data.is_question_shuffle
    ...
    async updataData() {
        if(this.login) {
            try {
              const callApi = await fetch("https://asia-southeast2-elearning-6871c.cloudfunctions.net/exams/" + this.dataItem, {
                method: 'PUT',
                headers: {'Content-Type': 'application/json','Authorization': 'Bearer ' + this.accessToken},
                body: JSON.stringify({
                  "name":this.name,
              ...
              if(finalRes.success) {
                this.$swal({ 
                  ...
                  if (confirmed.isConfirmed) {
                    console.log("Home");
                    window.location.href = "/lesson/exam/detail/" + this.dataItem
                  } else {
                    console.log("New");
  ...
  <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 xl:flex xl:items-center xl:justify-between">
    ...
    <div class="mt-5 flex xl:mt-0 xl:ml-4">
        <span class="hidden sm:block">
          <button @click="$router.push('/lesson/detail/' + this.parentID)" type="button" class="ml-3 inline-flex items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2 focus:ring-offset-gray-50">
