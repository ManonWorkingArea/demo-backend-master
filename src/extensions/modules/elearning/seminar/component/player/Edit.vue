  ...
  methods: {
    ...
    async getData() {
        if(storageManager.get('session','login')) {
            ...
            this.duration = RestReturn.data.duration
            ...
            try {
              ...
              if(finalRes.success) {
                this.$swal({ 
                  ...
                  confirmButtonText: "รายละเอียดเนื้อหา",
                  cancelButtonText: "บทเรียนหลัก",
                  closeOnConfirm: false,
                  closeOnCancel: false 
                }).then((confirmed) => {
                  if (confirmed.isConfirmed) {
                    console.log("Home");
                    window.location.href = '/lesson/player/detail/' + this.dataItem
                  } else {
                    console.log("New");
  ...
  <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 xl:flex xl:items-center xl:justify-between">
    ...
    <div class="mt-5 flex xl:mt-0 xl:ml-4">
        <span class="hidden sm:block">
          ...
          type="button" class="ml-3 inline-flex items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2 focus:ring-offset-gray-50">
          <font-awesome-icon :icon="['fas','home']" class="text-black text-[12px] mr-2"/>
          หลักสูตร
          </button>

          <button  
          @click="$router.push('/lesson/player/detail/' + this.dataItem)"
          type="button" class="ml-3 inline-flex items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2 focus:ring-offset-gray-50">
              ...
              <section aria-labelledby="payment-details-heading" class="relative" :data-content="'กำลังติดต่อฐานข้อมูล กรุณารอสักครู่.....'" :class="[(activeBlock?'isblock' : 'isunblock')]">
                  ...
                  <div class="divide-y divide-gray-200 overflow-hidden rounded-lg bg-white shadow mb-8">
                    <div class="px-4 py-5 sm:px-6">
                      ...
                      <p class="mt-1 text-sm text-gray-500">กรอกข้อมูลของบทเรียนให้ครบถ้วน - {{playerData.mode}}</p>
                    </div>
                    <div class="px-4 py-5 sm:p-6">
                          ...
                          <fieldset class="">
                            <div class="space-y-4 sm:flex sm:items-center sm:space-y-0 sm:space-x-10">
                              <div v-for="(typeItem) in playerType" :key="typeItem.id" class="flex items-center">
                                ...
                                <template v-if="playerData.mode==='sub'&&typeItem.code!='folder'">
                                ...
                                <template v-if="playerData.mode!='sub'&&playerData.type==='folder'&&typeItem.code==='folder'">
                                <input v-model="type" :id="typeItem.code" :value="typeItem.code" type="radio" class="mt-[-20px] h-4 w-4 border-gray-300 text-indigo-600 focus:ring-indigo-500">
                                <label :for="typeItem.code" class="ml-3 block text-sm font-bold text-gray-700">{{typeItem.slug}} <p class="text-gray-500 font-medium">{{typeItem.name}}</p></label>
                                </template>

                                <template v-if="playerData.mode!='sub'&&playerData.type==='folder'&&typeItem.code!='folder'">
                                  <input v-model="type" :id="typeItem.code" type="radio" class="mt-[-20px] h-4 w-4 border-gray-300 text-indigo-600 focus:ring-indigo-500 opacity-50" disabled readonly>
                                  <label :for="typeItem.code" class="ml-3 block text-sm font-bold text-gray-700">{{typeItem.slug}} <p class="text-gray-500 font-medium">{{typeItem.name}}</p></label>
                                  </template>

                                <template v-if="playerData.mode!='sub'&&playerData.type!='folder'">
                                <input v-model="type" :id="typeItem.code" :value="typeItem.code" type="radio" class="mt-[-20px] h-4 w-4 border-gray-300 text-indigo-600 focus:ring-indigo-500">
                                <label :for="typeItem.code" class="ml-3 block text-sm font-bold text-gray-700">{{typeItem.slug}} <p class="text-gray-500 font-medium">{{typeItem.name}}</p></label>
                                </template>
