...
export default {
      ...
      cert_area: [
        {
          id: "1",
          value:
            "สำนักวิจัยและพัฒนาการเกษตรเขตที่ 1 (เชียงใหม่ แพร่ น่าน ลำปาง เชียงราย แม่ฮ่องสอน)",
        },
        {
          id: "2",
          value:
            "สำนักวิจัยและพัฒนาการเกษตรเขตที่ 2 (พิษณุโลก พิจิตร สุโขทัย ตาก อุตรดิตถ์ เพชรบูรณ์)",
        },
        {
          id: "3",
          value:
            "สำนักวิจัยและพัฒนาการเกษตรเขตที่ 3 (ขอนแก่น หนองคาย กาฬสินธุ์ เลย อุดรธานี มุกดาหาร สกลนคร นครพนม ชัยภูมิ)",
        },
        {
          id: "4",
          value:
            "สำนักวิจัยและพัฒนาการเกษตรเขตที่ 4 (อุบลราชธานี นครราชสีมา สุรินทร์ มหาสารคาม ร้อยเอ็ด บุรีรัมย์ ศรีสะเกษ ยโสธร )",
        },
        {
          id: "5",
          value:
            "สำนักวิจัยและพัฒนาการเกษตรเขตที่ 5 (ชัยนาท กาญจนบุรี เพชรรัตน์เพชรบุรี ปทุมธานี ราชบุรี อุทัยธานี ลพบุรี นครปฐม นครสวรรค์ )",
        },
        {
          id: "6",
          value:
            "สำนักวิจัยและพัฒนาการเกษตรเขตที่ 6 (จันทบุรี ระยอง ปราจีนบุรี ฉะเชิงเทรา )",
        },
        {
          id: "7",
          value:
            "สำนักวิจัยและพัฒนาการเกษตรเขตที่ 7 (สุราษฎร์ธานี นครศรีธรรมราช กระบี่ ระนอง ภูเก็ต พังงา ชุมพร )",
        },
        {
          id: "8",
          value:
            "สำนักวิจัยและพัฒนาการเกษตรเขตที่ 8 (สงขลา ตาราง ตรัง นราธิวาส ปัตตานี ยะลา พัทลุง สตูล )",
        },
  ...
  methods: {
    ...
    redirectToExternalUrl() {
      window.open(
        "https://" + this.session.current.hostname + "/user/bypass/" + this.dataItem,
        "_blank"
      );
      //console.log('https://' + this.session.current.hostname + '/user/bypass/' + this.dataItem);
    },
    ...
    async updataData(image) {
      const callApi = await fetch(
        "https://gateway.cloudrestfulapi.com/api/user/" + this.profile._id,
        {
          method: "PUT",
          headers: {
    ...
    async updateEnrollAge(id) {
      try {
        const resAPI2 = await fetch(
          "https://gateway.cloudrestfulapi.com/api/enroll/" + id
        );
    ...
    async getData(force) {
      if (storageManager.get("session", "login")) {
        try {
          ...
          const resAPI = await fetch(
            `https://gateway.cloudrestfulapi.com/api/user/aggregate`,
            {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
...
<template>
          ...
          <ul class="flex space-x-2" id="tab-list">
            ...
            >
              <span class="hidden md:inline"
                ><font-awesome-icon :icon="['fas', tab.icon]" /> {{ tab.name }}
                <span
                  v-if="tab.code === 'enroll'"
                  class="text-xs bg-red-500 text-white py-0 px-1 rounded-md text-center"
                  >{{ enroll.length }}</span
                >
                <span
                  v-if="tab.code === 'material'"
                  class="text-xs bg-red-500 text-white py-0 px-1 rounded-md text-center"
                  >0</span
                >
              </span>
              <span class="md:hidden text-xs"
                ><font-awesome-icon :icon="['fas', tab.icon]" /> {{ tab.shortname }}</span
              >
              <!-- Add icon here -->
            </li>
          </ul>
          <select
            class="mb-3 md:hidden w-full border border-gray-300 rounded-md"
            id="tab-dropdown"
            @change="setActiveTab($event.target.value)"
          >
            <option v-for="tab in tabs" :key="tab.code" :value="tab.code">
              {{ tab.name }}
  ...
  <div
    class="mt-8 px-4 sm:px-6 lg:px-8"
  >
    <div class="space-y-6">
      <!-- Tab Content Container with modern styling -->
      <div class="bg-white rounded-lg shadow-sm border border-gray-200">
        <!-- Header Section for active tab -->
        <div class="px-4 py-3 border-b border-gray-200" v-if="activeTab">
          <div class="flex items-center justify-between">
            <div class="flex items-center space-x-3">
              <div class="w-8 h-8 bg-gradient-to-br from-purple-500 to-purple-600 rounded-lg flex items-center justify-center">
                <font-awesome-icon 
                  :icon="['fas', tabs.find(tab => tab.code === activeTab)?.icon || 'book']" 
                  class="text-white text-sm" 
                />
              </div>
              <div>
                <h2 class="text-base font-semibold text-gray-900">
                  {{ tabs.find(tab => tab.code === activeTab)?.name || 'ข้อมูล' }}
                </h2>
                <p class="text-xs text-gray-500">
                  {{ tabs.find(tab => tab.code === activeTab)?.description || 'จัดการข้อมูลนักเรียน' }}
                </p>
              </div>
            </div>
          </div>
          ...
          <div class="flex">
            ...
            <div class="ml-2">
              <p class="text-xs text-blue-700">
                <strong>หมายเหตุ:</strong> ข้อมูลในหน้านี้แสดงรายละเอียดของนักเรียน {{ this.profile.firstname + " " + this.profile.lastname }} และสามารถจัดการข้อมูลต่างๆ ได้ตามแท็บที่เลือก
              </p>
            </div>
          </div>
        </div>
