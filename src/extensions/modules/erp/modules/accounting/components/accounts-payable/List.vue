      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div class="flex items-center justify-between">
          <div class="flex items-center space-x-4">
            ...
            <div>
              <h1 class="text-2xl font-bold text-gray-900">Accounts Payable - เจ้าหนี้</h1>
              <p class="text-sm text-gray-600 mt-1">จัดการใบแจ้งหนี้และการชำระเงินให้กับผู้ขาย</p>
            </div>
          </div>
          <div class="flex items-center space-x-3">
            <!-- Uncomment when route is available
            <router-link
              :to="{ name: 'accounting-payable-create' }"
    ...
    <div class="bg-white border-b border-gray-200">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
        <nav class="flex items-center space-x-2 text-sm">
          ...
          <i class="fas fa-chevron-right text-gray-400 text-xs"></i>
          <span class="text-gray-900 font-medium">Accounts Payable</span>
        </nav>
      </div>
    </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div class="bg-gradient-to-br from-blue-500 to-indigo-600 rounded-lg shadow-sm hover:shadow-md transition-shadow p-6 text-white">
          <div class="flex items-center justify-between">
            <div>
              <p class="text-blue-100 text-sm font-medium mb-1">Total Payables</p>
              <p class="text-3xl font-bold">{{ summary.total_payables }}</p>
            </div>
        ...
        <div class="bg-gradient-to-br from-green-500 to-emerald-600 rounded-lg shadow-sm hover:shadow-md transition-shadow p-6 text-white">
          <div class="flex items-center justify-between">
            <div>
              <p class="text-green-100 text-sm font-medium mb-1">Paid This Month</p>
              <p class="text-3xl font-bold">{{ formatCurrency(summary.paid_this_month) }}</p>
            </div>
            <div class="bg-white bg-opacity-20 rounded-lg p-3">
      ...
      <div class="bg-white rounded-lg shadow-sm overflow-hidden">
        <div class="overflow-x-auto">
          <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              ...
              <tr v-if="loading">
                <td colspan="9" class="px-6 py-12 text-center">
                  <div class="flex flex-col items-center justify-center">
                    ...
                    <p class="text-gray-500">Loading payables...</p>
                  </div>
                </td>
              </tr>

              <!-- Empty State -->
              <tr v-else-if="filteredPayables.length === 0">
                <td colspan="9" class="px-6 py-12 text-center">
                  <div class="flex flex-col items-center justify-center">
                    <i class="fas fa-file-invoice text-gray-300 text-5xl mb-4"></i>
                    <p class="text-gray-500 text-lg font-medium">No payables found</p>
                    <p class="text-gray-400 text-sm mt-2">Create your first vendor invoice</p>
                    <!-- Uncomment when route is available
                    <router-link
                      :to="{ name: 'accounting-payable-create' }"
              ...
              <!-- Data Rows -->
              <tr v-else v-for="payable in paginatedPayables" :key="payable.id" class="hover:bg-gray-50 transition-colors">
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <div class="text-sm" :class="isOverdue(payable.due_date) ? 'text-red-600 font-medium' : 'text-gray-500'">
                ...
                <td class="px-6 py-4 whitespace-nowrap text-center">
                  <span :class="getStatusBadgeClass(payable.status)">
                ...
                <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                  <div class="flex items-center justify-center space-x-2">
                    ...
                    <button
                      v-if="payable.status !== 'paid' && payable.status !== 'cancelled'"
                      @click="makePayment(payable)"
        ...
        <div class="bg-white px-6 py-4 border-t border-gray-200 flex items-center justify-between">
          <div class="text-sm text-gray-700">
            ...
            of 
            <span class="font-medium">{{ filteredPayables.length }}</span>
            payables
          </div>
...
export default {
  ...
  computed: {
    filteredPayables() {
      ...
      if (this.filters.search) {
        ...
        filtered = filtered.filter(payable => 
          payable.invoice_number.toLowerCase().includes(searchLower) ||
          payable.vendor_name.toLowerCase().includes(searchLower)
        )
      }

      // Vendor filter
      if (this.filters.vendor_id) {
        filtered = filtered.filter(payable => payable.vendor_id === this.filters.vendor_id)
      }
      ...
      if (this.filters.due_date) {
        filtered = filtered.filter(payable => payable.due_date === this.filters.due_date)
      }
    ...
    paginatedPayables() {
      ...
      return this.filteredPayables.slice(start, end)
    },
    totalPages() {
      return Math.ceil(this.filteredPayables.length / this.pagination.per_page)
    },
    displayedPages() {
      const pages = []
    ...
    paginationEnd() {
      return Math.min(this.pagination.current_page * this.pagination.per_page, this.filteredPayables.length)
    },
    hasActiveFilters() {
      return this.filters.search || this.filters.vendor_id || this.filters.status || this.filters.due_date
    }
  },
  methods: {
    async loadPayables() {
      ...
      try {
        ...
        this.payables = [
          ...
          {
            id: 2,
            invoice_number: 'AP-2024-002',
            vendor_id: 2,
            vendor_name: 'XYZ Trading',
        ...
        this.$swal({
          icon: 'error',
          title: 'Error',
          text: 'Failed to load accounts payable'
        })
      } finally {
        this.loading = false
      }
    ...
    loadSummary() {
      this.summary.total_payables = this.payables.length
      this.summary.unpaid_amount = this.payables
        .filter(p => p.status !== 'paid' && p.status !== 'cancelled')
        .reduce((sum, p) => sum + p.balance, 0)
      this.summary.overdue_amount = this.payables
        .filter(p => p.status === 'overdue')
        .reduce((sum, p) => sum + p.balance, 0)
      
      const thisMonth = new Date().toISOString().slice(0, 7)
      this.summary.paid_this_month = this.payables
        .filter(p => p.status === 'paid' && p.invoice_date.startsWith(thisMonth))
    ...
    viewPayable(payable) {
      this.$swal({
        icon: 'info',
        title: 'View Payable',
        text: `View details for ${payable.invoice_number}`
      })
    },
    editPayable(payable) {
      this.$swal({
        icon: 'info',
        title: 'Edit Payable',
        text: `Edit ${payable.invoice_number}`
      })
    },
    makePayment(payable) {
      this.$swal({
        icon: 'info',
        title: 'Make Payment',
        text: `Make payment for ${payable.invoice_number}`
      })
    },
    deletePayable(payable) {
      this.$swal({
        title: 'Are you sure?',
        text: `Delete payable ${payable.invoice_number}?`,
        icon: 'warning',
        ...
        if (result.isConfirmed) {
          // Delete logic here
          this.$swal('Deleted!', 'Payable has been deleted.', 'success')
        }
      })
    }
  },
