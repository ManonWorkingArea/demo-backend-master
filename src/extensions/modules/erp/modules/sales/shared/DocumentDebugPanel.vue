              ...
              <div class="summary-content">
                ...
                <div class="summary-status">
                  <span :class="summary.status">
                    {{ summary.status === 'found' ? '✓ Linked' : '✗ Not Created' }}
                  </span>
                </div>
              </div>
              <button 
...
export default {
  ...
  setup(props) {
    ...
    const loadLinkedDocuments = async () => {
      ...
      if (!serviceInstance.value) {
        console.error('[Debug Panel] ❌ SalesService not available after initialization')
        return
      }
      
      const doc = props.currentDocument
      if (!doc) {
        console.warn('[Debug Panel] ⚠️ No current document provided')
        return
      }
      
      const docId = doc._id || doc.id
      if (!docId) {
        console.warn('[Debug Panel] ⚠️ Document has no ID')
        return
      }
      ...
      try {
        console.log('[Debug Panel] 🔍 Loading linked documents...')
        console.log('  - Document Type:', props.documentType)
        console.log('  - Document ID:', docId)
        
        let linkedData = null
        
        // ใช้ฟังก์ชันใหม่จาก SalesService
        if (props.documentType === 'Quotation') {
          console.log('[Debug Panel] 📋 Fetching Quotation linked documents...')
          
          // Check if method exists
          if (!serviceInstance.value.getQuotationWithLinkedDocuments) {
            console.error('[Debug Panel] ❌ Method getQuotationWithLinkedDocuments not found in salesService')
            return
          }
          
          linkedData = await serviceInstance.value.getQuotationWithLinkedDocuments(docId)
          console.log('[Debug Panel] 📦 Quotation linked data:', linkedData)
          ...
          if (linkedData.links.hasInvoice && linkedData.invoice) {
            console.log('[Debug Panel] ✅ Invoice found:', linkedData.invoice.invoice_number)
            ...
            console.log('[Debug Panel] ❌ Invoice not found')
            links.push({
              type: 'Invoice',
              status: 'not-found'
            })
          ...
          if (linkedData.links.hasSalesOrder && linkedData.salesOrder) {
            console.log('[Debug Panel] ✅ Sales Order found:', linkedData.salesOrder.order_number || linkedData.salesOrder.orderNumber)
            ...
            console.log('[Debug Panel] ❌ Sales Order not found')
            links.push({
              type: 'Sales Order',
              status: 'not-found'
            })
          ...
          console.log('[Debug Panel] 🧾 Fetching Invoice linked documents...')
          
          // Check if method exists
          if (!serviceInstance.value.getInvoiceWithLinkedDocuments) {
            console.error('[Debug Panel] ❌ Method getInvoiceWithLinkedDocuments not found in salesService')
            return
          }
          
          linkedData = await serviceInstance.value.getInvoiceWithLinkedDocuments(docId)
          console.log('[Debug Panel] 📦 Invoice linked data:', linkedData)
          ...
          if (linkedData.links.hasQuotation && linkedData.quotation) {
            console.log('[Debug Panel] ✅ Quotation found:', linkedData.quotation.quote_number || linkedData.quotation.quotation_number)
            quotationData.value = linkedData.quotation
            links.push({
              type: 'Quotation',
            ...
            console.log('[Debug Panel] ❌ Quotation not found')
            links.push({
              type: 'Quotation',
              status: 'not-found'
            })
          ...
          console.log('[Debug Panel] 🛒 Fetching Sales Order linked documents...')
          
          // Check if method exists
          if (!serviceInstance.value.getSalesOrderWithLinkedDocuments) {
            console.error('[Debug Panel] ❌ Method getSalesOrderWithLinkedDocuments not found in salesService')
            return
          }
          
          linkedData = await serviceInstance.value.getSalesOrderWithLinkedDocuments(docId)
          console.log('[Debug Panel] 📦 Sales Order linked data:', linkedData)
          
          // Set current document as sales order
          salesOrderData.value = linkedData.salesOrder
        ...
        console.log('[Debug Panel] ✅ Linked documents loaded:', links.length, 'documents')
        console.log('[Debug Panel] 📋 Final links array:', links)
        
      } catch (error) {
        console.error('[Debug Panel] ❌ Error loading linked documents:', error)
