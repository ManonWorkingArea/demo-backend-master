<template>
  ...
  <div class="mx-auto max-w-7xl px-4 mt-5 mb-8 sm:px-6">
    ...
    <!-- <div>
      ...
      <button @click="convertToTable">Convert to Table</button>
      <table v-if="tableData.length > 0" class="border border-collapse">
        <tbody>
          <tr v-for="(row, rowIndex) in tableData" :key="rowIndex">
            <td class="border border-gray-300 px-4 py-2">{{ rowIndex + 1 }}</td>
            <td v-for="(column, colIndex) in row" :key="colIndex" class="border border-gray-300 px-4 py-2">{{ column }}</td>
          </tr>
        </tbody>
      </table>
  ...
  <div v-if="showRenameDialog" class="fixed inset-0 flex items-center justify-center z-50">
    ...
    <div class="bg-white rounded p-5 z-10">
      ...
      <div class="flex flex-col space-y-2">
        ...
        <!-- Checkbox using v-model -->
        <div class="flex items-center space-x-2">
          <input v-model="selectedCategory.visible" type="checkbox" id="checkbox">
