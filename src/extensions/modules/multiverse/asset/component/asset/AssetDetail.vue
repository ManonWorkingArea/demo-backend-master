...
export default {
    ...
    methods: {
      ...
      async getData() {
        if(storageManager.get('session','login')) {
          try {
            ...
            const resAPI          = await fetch("https://gateway.cloudrestfulapi.com/api/hostname/" + item,
            {
              method: 'GET',
            ...
            if(resAPI.status === 200) {
              const childRes        = await this.getAdminData()
              console.log("childRes",childRes);
              if(childRes) {
                const unitRes       = await this.getUnitData()
                console.log("unitRes",unitRes);

                if(unitRes) {
                  this.unitData    = unitRes;
                  this.assetAdmin  = childRes;
      ...
      async getAdminData() {
        if(storageManager.get('session','login')) {
          try {
            ...
            const RestReturn   = await resAPI.json();
            console.log("getAdminData",RestReturn);
      ...
      async unassignAdmin(id) {
        try {
          this.activeBlock = true;
          const resAPI = await fetch("https://gateway.cloudrestfulapi.com/api/user/" + id + "/asset", {
            method: 'POST',
            headers: {'Content-Type':'application/json','client-token-key':this.hostkey},
            body: JSON.stringify({
              action: "remove",
          ...
          if(resAPI.status === 200) {
            //window.location.href = "/asset/detail/" + this.dataItem
          }
...
<template>
  ...
  <div v-if="loading_sources" class="mt-8 mx-auto max-w-3xl px-4 sm:px-6 md:flex md:items-center md:justify-between md:space-x-5 lg:max-w-7xl lg:px-8">
    ...
    <div class="justify-stretch mt-6 flex flex-col-reverse space-y-4 space-y-reverse sm:flex-row-reverse sm:justify-end sm:space-y-0 sm:space-x-3 sm:space-x-reverse md:mt-0 md:flex-row md:space-x-3">
      ...
      <button 
      @click="$router.push('/asset/edit/' + this.dataItem)"
      type="button" 
      ...
      <template v-if="assetAdmin.length === 0">
        <button
        @click="$router.push('/asset/admin/'+this.dataItem)"
        type="button" 
        class="inline-flex items-center justify-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-gray-100">
      ...
      <section aria-labelledby="applicant-information-title">
        <div class="bg-white shadow sm:rounded-lg">
          <div class="px-4 py-5 sm:px-6">
            <h2 id="applicant-information-title" class="text-lg font-medium leading-6 text-gray-900"><font-awesome-icon :icon="['fas','box']"/> Asset Information</h2>
      ...
      <section aria-labelledby="notes-title">
        <div class="bg-white shadow sm:overflow-hidden sm:rounded-lg">
          <div class="divide-y divide-gray-200">
            <div class="px-4 py-5 sm:px-6">
              <h2 id="notes-title" class="text-lg font-medium text-gray-900"><font-awesome-icon :icon="['fas','box-open']"/> Unit</h2>
            </div>
            <div class="px-4 py-6 sm:px-6">
              <ul role="list" class="space-y-8">
                

                <li v-for="(unit,index) in unitData" :key="index">
                  <div class="flex space-x-3">
                    <div class="flex-shrink-0">
                      <img class="h-10 w-10 rounded-full" :src="unit.loginLogo" alt="">
                    </div>
                    <div>
                      <div class="text-sm">
                        <a href="#" class="font-semibold text-gray-900">{{ unit.siteName }}</a>
                      </div>
                      <div class="mt-1 text-sm text-gray-700">
                        <p><font-awesome-icon :icon="['fas','globe']" class="text-gray-900 mt-[3px] mr-1 text-md"/> {{ unit.hostname }}</p>
                      </div>
                      <div class="mt-2 space-x-2 text-sm">

                        <span v-for="(plugin,index) in unit.plugins" :key="index" class="inline-block bg-indigo-100 rounded-sm px-2 py-1 text-xs font-medium text-gray-700 mr-1 mb-1">
                          {{ plugin }}
                        </span>
      ...
      <div class="bg-white px-4 py-5 shadow sm:rounded-lg sm:px-6">
        ...
        <div class="mb-5">
          ...
          <ul role="list" class="mt-2 divide-y divide-gray-200 border-t border-b border-gray-200">
              ...
              <li v-for="(admin,index) in assetAdmin" :key="index" class="flex items-center justify-between py-3">
              ...
              <template v-if="assetAdmin.length === 0">
                <li class="flex items-center justify-between py-2">
                  <button type="button" class="group -ml-1 flex items-center rounded-md bg-white p-1 focus:outline-none focus:ring-2 focus:ring-indigo-500">
                    ...
                    <span class="ml-4 text-sm font-medium text-indigo-600 group-hover:text-indigo-500" @click="$router.push('/asset/admin/'+this.dataItem)">เพิ่มเจ้าหน้าที่</span>
                  </button>
                </li>
              </template>
