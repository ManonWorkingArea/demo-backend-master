...
export default {
    ...
    return {
      ...
      loginBg: '',
      dashboardBg: '',
      dashboardGradientColor1: '#1e3a8a',
      dashboardGradientColor2: '#4338ca',
      sitePlugin: '',
      id: '',
      s3Key: '',
      s3Endpoint: '',
  ...
  methods: {
    ...
    selectFileTrigger(payload) {
      ...
      if(payload!=undefined) {
        ...
        } else if (this.selectedImage === 'dashboardBg') {
          this.dashboardBg = payload.file;
          this.updataCover(payload.file, 'dashboardBg');
    ...
    async updataCover(cover, image) {
      ...
      if (image === 'siteLogo') {
        ...
        this.loginBg = cover;
      } else if (image === 'dashboardBg') {
        this.dashboardBg = cover;
    ...
    async fetchData() {
      try {
        ...
        this.dashboardBg        = data.dashboardBg;
        this.dashboardGradientColor1 = data.dashboardGradientColor1 || '#1e3a8a';
        this.dashboardGradientColor2 = data.dashboardGradientColor2 || '#4338ca';
        ...
        const response = await fetch('https://gateway.cloudrestfulapi.com/api/hostname/' + this.id, {
          ...
          body: JSON.stringify({
            data: {
              ...
              dashboardGradientColor1: this.dashboardGradientColor1,
              dashboardGradientColor2: this.dashboardGradientColor2,
                        ...
                        <section aria-labelledby="payment-details-heading" class="relative" :data-content="'กำลังติดต่อฐานข้อมูล กรุณารอสักครู่.....'" :class="[(activeBlock?'isblock' : 'isunblock')]">
                          ...
                          <div class="shadow sm:overflow-hidden sm:rounded-md">
                              <div class="bg-white py-6 px-4 sm:p-6">
                                ...
                                <div class="mt-6 flex gap-6">
                                  <div class="w-1/2 bg-gray-200/80 rounded-lg">
                                    <button @click="OpenFileBrowser('dashboardBg')"
                                      v-bind:style="{ 'background-image': 'url(' + this.dashboardBg + ')' }"
                                      type="button"
                                      class="relative bg-cover block w-full h-full rounded-lg border-2 border-dashed border-gray-300 p-6 text-center hover:border-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2"
                                      style="background-size: cover; background-position: center;">
                                      <div class="p-3 bg-white bg-opacity-80 rounded-lg">
                                        <font-awesome-icon :icon="['fas','cloud-upload']" class="mx-auto h-12 w-12 text-gray-600"/>
                                        <span class="mt-2 block text-sm font-semibold text-gray-600">เปลี่ยน Dashboard Background</span>
                                      </div>
                                    </button>
                                  </div>
                                  <div class="w-1/2">
                                    <div class="grid grid-cols-1 md:grid-cols-1 gap-6">
                                      <div>
                                        <label for="dashboardBg" class="block text-sm font-semibold text-gray-700">Dashboard Background</label>
                                        <input v-model="dashboardBg" type="text" name="dashboardBg" id="dashboardBg" autocomplete="dashboardBg" readonly :class="getClasses('input-plaintext')" class="mt-1 block w-full rounded-md py-2 px-3">
                                        <p class="mt-1 text-sm text-gray-500">Upload a JPEG/PNG image (max 5MB) for dashboard background.</p>
                                      </div>
                                    </div>
                                  </div>
                                </div>
                                ...
                                <div class="mt-6 grid grid-cols-2 gap-6">
                                  <div>
                                    <label for="dashboardGradientColor1" class="block text-sm font-semibold text-gray-700">Dashboard Gradient Color 1</label>
                                    <div class="mt-2 flex items-center gap-3">
                                      <input 
                                        v-model="dashboardGradientColor1" 
                                        type="color" 
                                        id="dashboardGradientColor1" 
                                        class="h-10 w-20 rounded border border-gray-300 cursor-pointer"
                                      />
                                      <input 
                                        v-model="dashboardGradientColor1" 
                                        type="text" 
                                        class="flex-1 rounded-md border border-gray-300 py-2 px-3 shadow-sm focus:border-gray-900 focus:outline-none focus:ring-gray-900 sm:text-sm"
                                        placeholder="rgba(30, 58, 138, 0.85)"
                                      />
                                    </div>
                                    <p class="mt-1 text-sm text-gray-500">First gradient color for dashboard overlay (top-left)</p>
                                  </div>

                                  <div>
                                    <label for="dashboardGradientColor2" class="block text-sm font-semibold text-gray-700">Dashboard Gradient Color 2</label>
                                    <div class="mt-2 flex items-center gap-3">
                                      <input 
                                        v-model="dashboardGradientColor2" 
                                        type="color" 
                                        id="dashboardGradientColor2" 
                                        class="h-10 w-20 rounded border border-gray-300 cursor-pointer"
                                      />
                                      <input 
                                        v-model="dashboardGradientColor2" 
                                        type="text" 
                                        class="flex-1 rounded-md border border-gray-300 py-2 px-3 shadow-sm focus:border-gray-900 focus:outline-none focus:ring-gray-900 sm:text-sm"
                                        placeholder="rgba(67, 56, 202, 0.7)"
                                      />
                                    </div>
                                    <p class="mt-1 text-sm text-gray-500">Second gradient color for dashboard overlay (bottom-right)</p>
                                  </div>
                                </div>
