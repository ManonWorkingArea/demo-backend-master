      ...
      async assignAdmin(id) {
        try {
          ...
          if(resAPI.status === 200) {
            window.location.href = "/origin/collection/detail/" + this.dataItem
          }
                ...
                <table class="min-w-full divide-y divide-gray-300">
                  ...
                  <tbody class="divide-y divide-gray-200 bg-white">
                    ...
                    <tr v-for="(superadmin, index) in listItems" :key="superadmin._id">
                      ...
                      <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{{ superadmin.collection ? (superadmin.collection.length > 0 ? superadmin.collection.length : '0') : '0' }} Collection</td>
                      <td class="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-md font-medium sm:pr-6">
                        ...
                        <template v-if="!superadmin.collection || superadmin.collection.length === 0">
                          ...
                          <button @click="deleteAdmin(superadmin._id)" type="button" class="inline-flex justify-center rounded-md border border-gray-300 bg-white px-2 py-2 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-gray-900 focus:ring-offset-2">
                        ...
                        <template v-else>
                          <span class="mr-2 text-sm text-red-500"><font-awesome-icon :icon="['fas','times']" class="text-red-500 mt-[3px] mr-2 text-md"/> Assign ไปที่ Collection อื่นแล้ว</span>
                        </template>
                      </td>
                    </tr>
