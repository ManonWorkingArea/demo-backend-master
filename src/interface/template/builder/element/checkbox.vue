  ...
  <div>
    ...
    <template v-if="mode === 'preview'">
      <form class="p-2 border rounded-md shadow-sm">
        <div class="mb-2">
          <label class="block popup-label font-semibold mb-1">{{ localItem.name || 'Checkbox Group' }}<span v-if="localItem.required" class="text-xs text-red-500 ml-1">* จำเป็น</span></label>
          <div :class="localItem.inline ? 'flex flex-wrap gap-x-4 gap-y-1' : 'space-y-1'">
            <div v-for="(option, index) in localItem.options" :key="option.id || index" class="flex items-center">
              <input
                ...
                :required="localItem.required && index === 0"
                class="popup-checkbox mr-1.5"
              />
        ...
        <div class="bg-white border-b border-gray-200 px-6 py-4 sticky top-0 z-10">
          <div class="flex items-center space-x-3">
            <div class="w-8 h-8 bg-blue-100 rounded-lg flex items-center justify-center">
              <i class="fas fa-check-square text-blue-600 h-4 w-4"></i>
            </div>
            <div>
              <h3 class="text-lg font-semibold text-gray-900">Checkbox Group Configuration</h3>
              <p class="text-sm text-gray-500">ตั้งค่าการแสดงผลของ checkbox group</p>
            </div>
          </div>
        </div>
        <div class="px-6 py-4 space-y-6">
          <div class="bg-white border border-gray-200 rounded-lg shadow-sm p-4 grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-3 mb-3">
            <div>
              ...
              <input v-model="localItem.name" type="text" class="w-full popup-input rounded-md border border-gray-300 p-2 focus:border-blue-500 focus:ring-2 focus:ring-blue-500" placeholder="e.g., Interests, Services"/>
              <span class="popup-helper text-gray-500 text-sm">This name will be used for the `name` attribute of the input fields.</span>
            </div>
            <div>
              <label class="popup-label block font-semibold mb-1">Required Error Message:</label>
              <input v-model="localItem.reqError" type="text" class="w-full popup-input rounded-md border border-gray-300 p-2 focus:border-blue-500 focus:ring-2 focus:ring-blue-500" placeholder="e.g., Please select at least one option"/>
            </div>
          </div>
          <div class="bg-white border border-gray-200 rounded-lg shadow-sm p-4 mt-3">
            <label class="popup-label block mb-2 font-semibold">Checkbox Options:</label>
            ...
            <div v-for="(option, index) in localItem.options" :key="option.id" class="option-item mb-2 p-2.5 border rounded-md" :class="[index % 2 === 0 ? 'bg-white' : 'bg-gray-50']">
              ...
              <div class="flex items-center justify-end space-x-1.5 text-xs">
                <label class="flex items-center mr-auto">
                  <input v-model="option.checked" type="checkbox" class="popup-checkbox mr-1" />
                  <span class="popup-label text-xs">Checked by default</span>
                </label>
                <button @click="moveCheckboxOptionUp(index)" :disabled="index === 0" class="option-btn">
                  <font-awesome-icon :icon="['fas', 'arrow-up']"/>
                </button>
                <button @click="moveCheckboxOptionDown(index)" :disabled="index === localItem.options.length - 1" class="option-btn">
                  <font-awesome-icon :icon="['fas', 'arrow-down']"/>
                </button>
                <button @click="cloneCheckboxOption(index)" class="option-btn">
                  <font-awesome-icon :icon="['fas', 'clone']"/> Clone
                </button>
                <button @click="deleteCheckboxOption(index)" class="option-btn text-red-600 hover:text-red-700">
                  <font-awesome-icon :icon="['fas', 'trash']"/> Delete
                </button>
              </div>
            </div>
            <button @click="addCheckboxOption" class="mt-1 px-3 py-1.5 bg-blue-500 text-white rounded-md hover:bg-blue-600 text-sm">
          ...
          <div class="bg-white border border-gray-200 rounded-lg shadow-sm p-4 mt-4 pt-3 border-t border-gray-200">
            ...
            <div class="flex flex-wrap gap-x-6 gap-y-2">
              <div class="flex items-center">
                <input v-model="localItem.required" type="checkbox" id="checkbox_required" class="popup-checkbox mr-2" />
                <label for="checkbox_required" class="popup-label text-sm">Required</label>
              </div>
              <div class="flex items-center">
                <input v-model="localItem.inline" type="checkbox" id="checkbox_inline" class="popup-checkbox mr-2" />
                <label for="checkbox_inline" class="popup-label text-sm">Display Inline</label>
              </div>
            </div>
          </div>
        </div>
      ...
      handler(newItem) {
        ...
        if (parsedItem.options && Array.isArray(parsedItem.options)) {
            parsedItem.options = parsedItem.options.map(opt => ({
                 id: generateUid(), 
                 checked: opt.checked || false, // Ensure default for 'checked'
                 ...opt 
            }));
        } else {
            parsedItem.options = [];
  ...
  methods: {
    addCheckboxOption() {
      ...
      this.localItem.options.push({
        ...
        description: '',
        checked: false, // Default for new options
      });
    },
