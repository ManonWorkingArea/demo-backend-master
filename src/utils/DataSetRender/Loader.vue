<template>
    ...
    <template v-if="formRawData && formRawData.setData">
        <div v-for="(key, index) in Object.keys(formRawData.setData)" :key="index">
          <div v-for="(contactItem, contactIndex) in formRawData.setData[key]" :key="`contact_${index}_${contactIndex}`" class="w-full text-lg font-normal text-gray-700">
            <!-- <pre>{{ contactItem }}</pre> -->
            <multiple-data :value="contactItem" :set="key" @set-updated="handleSetDataUpdated"></multiple-data>
          </div>
        </div>
      </template>
      ...
      <div class="mt-5 mb-5">
          <address-data :addresses="addressArray" @address-updated="handleAddressUpdated" class="text-sm"></address-data>
      </div>
    ...
    // MultipleDataRender
    import MultipleData from '@/utils/DataSetRender/Multiple.vue';
    import AddressData from '@/utils/DataSetRender/Address.vue';


    export default {
      components: {
        AddressData,MultipleData,FormUpload
      },
      props: {
        formData: {
          type: Object,
